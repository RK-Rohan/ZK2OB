<?php
    ...
    function zkgetattendance($self) {
        ...
        try {
        @socket_recvfrom($self->zkclient, $self->data_recv, 1036, 0, $self->ip, $self->port);
        
        if ( getSizeAttendance($self) ) {
            $bytes = getSizeAttendance($self);

            while ( $bytes > 0  ) {
                @socket_recvfrom($self->zkclient, $data_recv, 1036, 0, $self->ip, $self->port);
                /////////////////////
                    $ret = @socket_recvfrom($self->zkclient, $data_recv, 1036, 0, $self->ip, $self->port);
                    if ($ret === false) {
                        throw Exception::createFromSocketResource($self->zkclient);
                    }
               ////////////////////////
            ...
            @socket_recvfrom($self->zkclient, $data_recv, 1028, 0, $self->ip, $self->port);
        }
        ...
        if ( count($self->attendancedata) > 0 ) {
            ...
            while ( strlen($attendancedata) > 40 ) {
                ...
                $uid = $u1+($u2*256);
                $id = intval( str_replace("\0", '', hex2bin( substr($u[1], 8, 24) ) ) );
                $state = hexdec( substr( $u[1], 56, 2 ) );
                $timestamp = decode_time( hexdec( reverseHex( substr($u[1], 58, 8) ) ) );
                $checktype =  hexdec( substr( $u[1], 66, 2 ) );
                ...
                array_push( $attendance, array( $uid, $id, $state, $timestamp, $checktype ) );
